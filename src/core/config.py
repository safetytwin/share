...
DEFAULT_CONFIG = {
    ...
    "api": {
        ...
        "allow_remote": os.environ.get('AI_ENV_API_ALLOW_REMOTE', "false").lower() == "true",
    ...
    "p2p": {
        ...
        "federation": {
            "enabled": os.environ.get('AI_ENV_FEDERATION_ENABLE', "false").lower() == "true",
    ...
    "resources": {
        ...
        "memory_reservation_percent": int(os.environ.get('AI_ENV_MEMORY_RESERVATION', 20)),
    ...
    "runtime": {
        ...
        "container": {
            "enabled": os.environ.get('AI_ENV_CONTAINER_ENABLE', "true").lower() == "true",
    ...
    "logging": {
        ...
        "file": os.environ.get('AI_ENV_LOG_FILE', str(CONFIG_DIR / "logs" / "ai-environment-manager.log")),
        "max_size": int(os.environ.get('AI_ENV_LOG_MAX_SIZE', 10 * 1024 * 1024)),  # 10 MB
...
class Config:
    ...
    def load(self) -> bool:
        ...
        if not self.config_file.exists():
            logger.info(f"Plik konfiguracyjny {self.config_file} nie istnieje, tworzenie domyślnej konfiguracji")
            return self.save()
    ...
    def _safe_config(self) -> Dict[str, Any]:
        ...
        if "p2p" in safe_config and "federation" in safe_config["p2p"] and "token" in safe_config["p2p"]["federation"]:
            safe_config["p2p"]["federation"]["token"] = "********" if safe_config["p2p"]["federation"]["token"] else ""

        if "integrations" in safe_config:
            if "git" in safe_config["integrations"] and "token" in safe_config["integrations"]["git"]:
                safe_config["integrations"]["git"]["token"] = "********" if safe_config["integrations"]["git"][
                    "token"] else ""

            if "ci" in safe_config["integrations"] and "token" in safe_config["integrations"]["ci"]:
                safe_config["integrations"]["ci"]["token"] = "********" if safe_config["integrations"]["ci"][
                    "token"] else ""
...
def ensure_directories():
    ...
    log_file = Path(config.get("logging.file", str(CONFIG_DIR / "logs" / "ai-environment-manager.log")))
...
def configure_logging():
    ...
    log_file = config.get("logging.file", str(CONFIG_DIR / "logs" / "ai-environment-manager.log"))
